...
from blockchain import mint_merchant_score, lookup_merchant_score
...
def process_scoring(csv_bytes: bytes) -> dict:
    ...
    final_score = int(np.clip(base_score + total_adj, 0, 100))
    ...
    audio_summary = {
        "en": f"Your TrustLedger credit score is {final_score} out of 100, which is {en_band}. Your primary growth driver is {en_factor}.",
        "hi": f"आपका ट्रस्टलेजर क्रेडिट स्कोर 100 में से {final_score} है, जो कि {hi_band} है। आपका मुख्य सकारात्मक कारक {hi_factor} है।"
    }
...
def get_preset_score(profile: str):
    ...
    if profile == 'injection':
        # Create a synthetic CSV containing a formula injection payload
        malicious_csv = (
        ...
        try:
            ...
            return process_scoring(csv_bytes)
        except ValueError as ve:
            raise HTTPException(status_code=400, detail=str(ve))
        except Exception as e:
